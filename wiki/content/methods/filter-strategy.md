...
5. **Exclusion direction is asymmetric.** High-cost comps exclude low-cost reroll carries (Nasus 3★ i3 in nova_95 = Bonk contamination). Low-cost reroll comps do NOT exclude high-cost carries (Fiora in Bonk = the player's cap board, a legitimate part of the comp's games). Reroll comps naturally transition to high-cost units in late game — that's their ceiling, not contamination.
6. **Item threshold follows game mechanics.** TFT gives ~9 components by stage 4 end ≈ 4-5 complete items. So: single carry → i3 (one unit absorbs all items); dual carry → i2 each (items distributed). Never use i3 on two units — that's 6+ items, only happens in extreme winning boards (survivorship bias maximized).
7. **Exclude hero augments from item analysis.** Hero augments give specific units special items — these are augment choices, not itemization decisions. They contaminate item Necessity rankings.
4. **Carry uniqueness determines filter complexity.** Unique carry = carry-only filter. Shared carry = need trait locks and exclusions. The taxonomy maps to one variable: how many comps share this carry?
